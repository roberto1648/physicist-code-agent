...
def main(
    ...
    wo=2.354,
    teval=[],
    system='multilevel', #'multiphoton multilevel',
    plot_all=False,
):
    ...
    if not are_transitions_resolvable(t, wij, wo,
        ...
        return np.zeros(shp, dtype='complex')
...
def calculate_dUt(VI=[[[]]], times=[]):
    ...
    du = -1j*dt/hbar*VI/2
    I = np.array(nt * [np.eye(nlevs)], dtype='complex')
    pade_numerator = I + du
...
def ut2uf(ut=[[[]]], t=[], wo=2.354):
    nlevs = np.shape(ut)[1]
    uf = np.zeros(np.shape(ut), dtype='complex')
    dt = t[1] - t[0]
